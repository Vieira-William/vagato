#!/usr/bin/env python3
"""
Dashboard 1:1 Pixel-Perfect FULL v5 - QA Verified
"""
...
(async () => {
  try {
    ...
    const ICONS = {
        ...
        external: '<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"></path><polyline points="15 3 21 3 21 9"></polyline><line x1="10" y1="14" x2="21" y2="3"></line></svg>'
    };
    ...
    function frame(opts) {
      ...
      // CRITICAL: Start small so AutoLayout HUG works correctly.
      // Figma defaults new frames to 100x100 which breaks HUG sizing.
      ...
      if (opts.dir) {
        ...
        // Default: HUG on both axes
        f.primaryAxisSizingMode = "AUTO";
        f.counterAxisSizingMode = "AUTO";

        // Override to FIXED when explicitly requested
        if (opts.dir === "HORIZONTAL") {
            if (opts.wFixed || opts.w) f.primaryAxisSizingMode = "FIXED";
            if (opts.hFixed || opts.h) f.counterAxisSizingMode = "FIXED";
            // But if hHug is explicit, override back to AUTO
            if (opts.hHug) f.counterAxisSizingMode = "AUTO";
            if (opts.wHug) f.primaryAxisSizingMode = "AUTO";
        } else { // VERTICAL
            if (opts.hFixed || opts.h) f.primaryAxisSizingMode = "FIXED";
            if (opts.wFixed || opts.w) f.counterAxisSizingMode = "FIXED";
    ...
    const CONTENT = frame({
        ...
        w: 1216, h: 736, wFixed: true, hFixed: true, pAll: 24, gap: 24
    });
    ...
    // QA Fix: 2 Grid Columns for VagaCards. Width = 920 total. Cards = (920 - 24) / 2 = 448px each.
    // E padding original é 20px com gap interno 16px.
    const GRID_WRAP = frame({
        name: "Grid Wrap", dir: "HORIZONTAL", wrap: true, gap: 24, w: 920, wFixed: true, hHug: true
    });

    const cardsData = [
      {title: "Product Designer Sênior — Design System", score: 92, source: "LinkedIn", e: "Neon", status: "Pendente", c: "#22c55e", st: "#f59e0b", srC: "#0ea5e9"},
      {title: "UX Designer Pleno — Growth & Experimentation", score: 85, source: "Indeed", e: "Nubank", status: "Pendente", c: "#22c55e", st: "#f59e0b", srC: "#f59e0b"},
      {title: "UX Lead — Seller Experience & Onboarding", score: 78, source: "Posts", e: "Mercado Livre", status: "Aplicada", c: "#6366f1", st: "#22c55e", srC: "#a855f7"},
      {title: "Product Designer — Mobile & Consumer Apps", score: 74, source: "LinkedIn", e: "Itaú", status: "Pendente", c: "#22c55e", st: "#f59e0b", srC: "#0ea5e9"},
      {title: "UX/UI Designer — Plataforma B2B SaaS", score: 71, source: "Indeed", e: "TOTVS", status: "Aplicada", c: "#6366f1", st: "#22c55e", srC: "#f59e0b"},
      {title: "Senior UX Designer — Fintech Products", score: 68, source: "Posts", e: "Creditas", status: "Pendente", c: "#f59e0b", st: "#f59e0b", srC: "#a855f7"}
    ];

    cardsData.forEach((cd, i) => {
        // Agora com HUG hMode e WFixed=448px (2 colunas)
        const cardBox = frame({
            name: `Card ${i}`, dir: "VERTICAL", bg: "#1a1a1f", stroke: "#2e2e33", sw: 1, r: 16,
            w: 448, wFixed: true, hHug: true, pAll: 20, gap: 16
        });
        ...
        const srcBadge = frame({name: "Fonte", dir:"HORIZONTAL", ai:"CENTER", jc:"CENTER", pX: 8, pY: 4, bg: cd.srC, bgOp:0.15, r: 12});
        srcBadge.appendChild(txt(cd.source, 10, "Regular", cd.srC));
        badgeWrap.appendChild(srcBadge);
        
        const dateTag = txt("Hoje", 10, "Regular", "#a1a1aa");
        badgeWrap.appendChild(dateTag);

        cardH1.appendChild(badgeWrap);
        
        const favIcon = getIcon("heart", 16, "#a1a1aa");
        ...
        tWrap.appendChild(txt(cd.title, 16, "Bold", "#ffffff", {wFill: true})); // Aumentado fonte do título
        tWrap.appendChild(txt(`${cd.e} • São Paulo, SP`, 12, "Regular", "#a1a1aa", {wFill: true}));
        ...
        // Spacer p/ empurrar o Footer
        cardBox.appendChild(frame({name:"div", w:1, h:8, wFixed:true, hFixed:true}));

        // Footer Actions
        const btnRow = frame({name: "Footer Actions", dir: "HORIZONTAL", jc: "SPACE_BETWEEN", ai: "CENTER"});
        btnRow.layoutAlign = "STRETCH";
        
        // Esquerda: Status
        const stWrap = frame({name: "Status", dir:"HORIZONTAL", pX:10, pY:4, bg:cd.st, bgOp:0.15, r:12, gap:6, ai:"CENTER"});
        stWrap.appendChild(getIcon("clock", 12, cd.st));
        stWrap.appendChild(txt(cd.status, 10, "Semi Bold", cd.st));
        btnRow.appendChild(stWrap);

        // Direita: Actions
        const actWrap = frame({name: "Actions", dir:"HORIZONTAL", gap:8, ai:"CENTER"});
        
        actWrap.appendChild(getIcon("external", 16, "#71717a"));
        ...
        actWrap.appendChild(aiBtn);

        const mBtn1 = frame({
            name: "Apply", dir: "HORIZONTAL", ai: "CENTER", jc: "CENTER", pX: 16, pY: 8, r: 8, stroke: "#4F46E5", sw: 1
        });
        mBtn1.appendChild(txt(cd.source === "LinkedIn" ? "Aplicar" : "Enviar DM", 12, "Medium", "#4F46E5"));
        actWrap.appendChild(mBtn1);

        btnRow.appendChild(actWrap);
    ...
    console.log("✅ Dashboard QA Verified (v5) injetado!");
    figma.notify("✅ Dashboard Vagas UX QA APPROVED RECRIADO (v5)", {timeout: 3000, error: false});
...
def inject():
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as r:
            print("✅ Pixel Perfect v5 (QA Verified) enviado para o Figma!")
            return True
