...
|--------|--------------|----------------------------|-------------------------|------------------------------|--------------------|
| **MacBook (Claude)** | 🟢 CONCLUÍDO | ✅ **PRD v19 — QA TOTAL: 47 issues corrigidos.** FASE 1: JWT middleware Supabase (supabase_auth.py), todos endpoints protegidos, interceptor Axios. FASE 2: sliders redistribuição proporcional, dark mode cards Configurações/Perfil, menu hambúrguer mobile (Sheet), logo mobile navega /dashboard. FASE 3-4: 35+ acentos PT-BR em toda plataforma (P1-P18, AD1-AD15, L1, etc). FASE 5: title "Vagato", 404 reescrito, email fallback, DB indexes. Build ✅ zero erros. Commit 2e79c4e. | **William** | Configurar SUPABASE_JWT_SECRET no .env para ativar JWT em produção. | 06/03/2026 |
